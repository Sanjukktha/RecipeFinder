...
const getS3Link = (uploadResults: UploadReturnType[] | null, location: string) => {
    ...
    if (filteredResult[0]?.uploaded) {
        // Use the same URL format as uploadAudioToS3 - include region for proper S3 access
        const bucketName = process.env.S3_BUCKET_NAME || 'smart-recipe-generator';
        const region = process.env.AWS_REGION || 'us-east-2';
        const trimmedLocation = location.trim(); // Remove any trailing whitespace
        return `https://${bucketName}.s3.${region}.amazonaws.com/${trimmedLocation}`;
    }
    return fallbackImg;
};
