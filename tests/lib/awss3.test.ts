...
import * as awss3 from "../../src/lib/awss3";
const { uploadImagesToS3, uploadAudioToS3 } = awss3;
...
describe('Uploading images to S3', () => {
    ...
    it('will handle failures in processing images', async () => {
        ...
        nock('https://openai-img-link-1')
            .get('/')
            .replyWithError({ message: 'ENOTFOUND', code: 'ENOTFOUND' });
...
describe('Uploading audio to S3', () => {
    ...
    it('will upload the audio to S3 successfully', async () => {
        ...
        expect(ans).toBe('https://stub-bucket-name.s3.us-east-2.amazonaws.com/audio/s3-audio-key')
    });
