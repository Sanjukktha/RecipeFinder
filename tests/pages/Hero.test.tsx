...
describe("The Hero Component",()=>{
    ...
    it('renders Recipe Finder text in the header', async () => {
        render(<Hero />)
        const recipeFinderText = await screen.findAllByText('Recipe Finder')
        const visibleRecipeFinder = recipeFinderText.find((node) => !node.classList.contains('sr-only'))
        expect(visibleRecipeFinder).toBeDefined();
    })
